...
const Checklist = () => {
    ...
    const fetchChecklist = async (id) => {
        try {
            const response = await axios.get(`http://localhost:8080/stress/checklist`, {
    ...
    const handleCheck = async (day) => {
        try {
            await axios.post(`http://localhost:8080/stress/checklist/check-day`, {
    ...
    return (
        <>
        <Navbar/>
        <Container style={{paddingTop:"60px", marginBottom:"10px"}}>
            <Row>
                <Col md={5} className="d-flex justify-content-center align-items-center">
                    <div>
                        <p>
                        "Ayo, mari kita praktikkan bersama sekarang! Ikuti intervensi ini selama 14 hari. Setiap hari, praktikkan Stress Coping Strategies pilihanmu dan ceklis pengalamanmu di daily checklist disamping!"
                        </p>
                        <img src={checklistImage} alt="Intervensi Cemas" className="img-fluid" style={{ maxWidth: '250px' }} />
                    </div>
                </Col>
                <Col md={7}>
                    <Card className="text-center" style={{backgroundColor:"#FFD5DF"}}>
                        <Card.Body style={{marginBottom:"50px"}}>
                            ...
                            {Array.from({ length: 14 }, (_, i) => i + 1).map((day) => (
                                <Button
                                    style={{marginTop:"15px", outlineColor:"#white", backgroundColor:"#F5A5AD"}}
                                    key={day}
