...
const HasilJawaban = () => {
  ...
  const [depresiChecklist, setDepresiChecklist] = useState([]);
  ...
  const getDepresiChecklist = async () => {
    try {
      const response = await axios.get("http://localhost:8080/depresi/checklist/semua");
      setDepresiChecklist(response.data);
    } catch (error) {
      console.error("Error fetching depresi checklist data:", error);
    }
  };
    ...
    <AdminLayout>
      <div>
        <Breadcrumb className="mt-3">
          ...
          <Breadcrumb.Item active>Checklist Depresi </Breadcrumb.Item>
        </Breadcrumb>
          ...
          <Card.Body>
            ...
            <div className="table-responsive" style={{ overflowY: "auto", maxHeight: "450px" }}>
              <Table striped bordered hover>
                ...
                <tbody>
                  {depresiChecklist.map((item, index) => (
                    <tr key={index}>
                      <td>{index + 1}</td>
