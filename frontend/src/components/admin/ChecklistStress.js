...
const HasilJawaban = () => {
  ...
  const getStressChecklist = async () => {
    try {
      const response = await axios.get("http://localhost:8080/stress/checklist/semua");
      setStressChecklist(response.data);
    } catch (error) {
      console.error("Error fetching stress checklist data:", error);
    }
  };
    ...
    <AdminLayout>
      <div>
        <Breadcrumb className="mt-3">
          ...
          <Breadcrumb.Item active>Checklist Stress </Breadcrumb.Item>
        </Breadcrumb>
          ...
          <Card.Body>
            ...
            <div className="table-responsive" style={{ overflowY: "auto", maxHeight: "450px" }}>
              <Table striped bordered hover>
                ...
                <tbody>
                  {stressChecklist.map((item, index) => (
                    <tr key={index}>
                      <td>{index + 1}</td>
